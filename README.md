...
- Press K in game to select mode (off / full / solids only). For more information check out the Atomic Frontier video https://www.youtube.com/@atomicfrontier 
...
7. You should now be able to load up a world and play. Press "K" to select the mode.
...
- To help with this, the easured flip rates in the actual experiment were:
  ...
  - Fun fact! The radiation damadge is entirely random both in type (i.e. 0->1 is as likely as 1->0) and position (no bit within a byte, or byte within the chip is any more or less likely). I was kinda hoping it would be more geometric.
- In addition to the pretty cheat sheet, I also made an interactive one to help with the route plan (click one cell and then its bit-flip neighbors are highlighted). We should probably add something similar into the mod but I didn't have the time.
